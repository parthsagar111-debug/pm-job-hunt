"""
cos_eir_hosted.py — Chief of Staff / EIR Job Alert (hosted / GitHub Actions version)
======================================================================================
...
Output:   Google Sheet (Apply / Maybe / Skip tabs) — dedicated COS_EIR_SPREADSHEET_ID sheet
...
Dedup and error handling follow the pattern fixed in pm_eval_hosted.py after the
August 2026 cost incident:
  - Dedup happens BEFORE evaluation (load_seen_urls from the Sheet up front), not
    just at write time — evaluating already-seen jobs burns API tokens for nothing.
  - A failed API call returns decision "Error", not "Skip" — Error jobs are excluded
    from the Sheet write so they aren't permanently marked as seen, and get retried
    next run instead of silently blackholed.
  - 3 consecutive Error decisions abort the batch early (dead key / no funds / outage)
...
import core_cos_eir_hosted as core
...
def evaluate_batch(jobs: list) -> tuple[list, bool]:
    ...
    evaluated_jobs only contains jobs that got a real decision — jobs whose API call
    failed (decision == "Error") are left out so they aren't written to the Sheet and
    aren't marked as seen; they'll be re-fetched and retried next run. If several calls
    in a row fail, stop early instead of burning through the whole batch.
    """
    total = len(jobs)
    ok_jobs = []
...
def main():
    ...
    print(f"\n{'='*55}")
    print(f"  CHIEF OF STAFF / EIR EVAL (HOSTED) — 24h catch-up")
    print(f"  [{now}]")
    ...
    if not all_jobs:
        ...
        run_summary("Chief of Staff / EIR", 0, 0, 0)
        return
    ...
    if not evaluated_jobs:
        ...
        run_summary("Chief of Staff / EIR — FAILED", 0, 0, 0)
        try:
            close_browser()
        except Exception:
            pass
    ...
    label = "Chief of Staff / EIR — partial (API errors)" if aborted else "Chief of Staff / EIR"
